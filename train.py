...
if __name__ == '__main__':
    ...
    parser.add_argument('--stratify', type=str2bool, default=False, help='stratify the dataset')
    parser.add_argument('--class_power', type=float, default=None, help='power for class stratification')
    parser.add_argument('--sens_power', type=float, default=None, help='power for sens stratification')
    ...
    features, labels, sens, edge_index, idx_train, idx_val, idx_test = load_dataset(args.dataset, device, stratify=args.stratify, \
                                                                                    class_power=args.class_power, sens_power=args.sens_power, \
                                                                                    seed=args.seed)
    save_folder = f'{args.dataset}_{args.model}'
    if args.stratify:
        save_folder += f'stratify_class_{args.class_power}_sens_{args.sens_power}_{args.seed}'
    ...
    for run in range(args.num_runs):
        ...
        with open(f'results/{save_folder}/iter_{run}.pkl'.format(run), 'wb') as f:
            pickle.dump(results, f)
